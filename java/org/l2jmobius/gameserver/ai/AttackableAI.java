...
public class AttackableAI extends CreatureAI
{
	...
	protected void thinkAttack()
	{
		...
		if (Config.AGGRO_DISTANCE_CHECK_ENABLED && npc.isMonster() && !npc.isWalker() && !(npc instanceof GrandBoss))
		{
			...
			if ((spawn != null) && (npc.calculateDistance2D(spawn.getLocation()) > (spawn.getChaseRange() > 0 ? Math.max(Config.MAX_DRIFT_RANGE, spawn.getChaseRange()) : (npc.isRaid() || (npc instanceof GrandBoss) ? Config.AGGRO_DISTANCE_CHECK_RAID_RANGE : Config.AGGRO_DISTANCE_CHECK_RANGE))))
			{
				if ((Config.AGGRO_DISTANCE_CHECK_RAIDS || !npc.isRaid() || (npc instanceof GrandBoss)) && (Config.AGGRO_DISTANCE_CHECK_INSTANCES || (npc.getInstanceId() == 0)))
				{
